...
After installing the [Trivy Operator](https://aquasecurity.github.io/trivy-operator), an open-source security scanner, into my Kubernetes cluster, I noticed the reports could get pretty big and they would need some tool to parse them. 

My preference was to view the data in `Argo CD`, largely out of having visibility into the report data in `Argo CD` which is a almost a default Kubernetes interface for cluster admins and developers.
...
A quick but dirty test can be achieved by copying the extension via `kubectl cp` in the Argo CD servers `/tmp/extension/` path , e.g. on a minikube or other non-production cluster.
